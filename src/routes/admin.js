...
router.get("/download-stats", adminAuth, async (req, res) => {
  try {
    ...
    const [total, today, last7days, byPlatform, byFormat, byQuality, failed, cacheHitsAll, cacheHitsToday] = await Promise.all([
      ...
      Download.aggregate([{ $match: { updatedAt: { $gte: last24h } } }, { $group: { _id: null, total: { $sum: "$cacheHitCount" } } }]),
    ]);
    ...
    res.json({
      total: total + totalCacheHits,
      today: today + todayCacheHits,
      last7days,
      byPlatform,
      byFormat,
      byQuality,
      ...
      cacheHitRate: totalCacheHits ? Math.round((totalCacheHits / (total + totalCacheHits)) * 100) : 0,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
