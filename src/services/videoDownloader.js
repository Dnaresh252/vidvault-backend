...
const crypto = require("crypto");
const { spawn } = require("child_process");
const cookieManager = require("./cookieManager"); // 🔥 CRITICAL FIX
...
class VideoDownloaderService {
  ...
  async downloadVideo(options = {}) {
    ...
    try {
      ...
      // 🔥 CRITICAL: Use retry logic for download
      ...
      try {
        ...
        if (
          ...
          if (!cookieStatus.hasCookies) {
            ...
            console.error(
              `💡 Add YOUTUBE_COOKIES environment variable in Railway`,
            );
  ...
  // 🔥 CRITICAL FIX: Add cookies to yt-dlp options
  buildDownloadOptions({ quality, format, audioOnly }) {
    const options = [];

    // 🔥 STEP 1: ADD COOKIES FIRST (MOST IMPORTANT)
    const hasCookies = cookieManager.addCookieOptions(options);

    if (!hasCookies) {
      console.log(
        "⚠️  WARNING: Downloading without cookies - bot detection may occur!",
      );
    }
  ...
  getUserFriendlyError(errorMessage) {
    const errorMap = {
      "sign in":
        "⚠️ YouTube bot detection! Please contact support to enable cookie authentication.",
      bot: "⚠️ YouTube bot detection! Please contact support to enable cookie authentication.",
      private: "This video is private and cannot be downloaded.",
      unavailable: "This video is no longer available.",
      removed: "This video has been removed.",
      "age-restricted":
  ...
  async getVideoMetadata(url, platform) {
    try {
      ...
      // 🔥 ADD COOKIES FOR METADATA EXTRACTION TOO
