...
const crypto = require("crypto");
const { spawn } = require("child_process");
const cookieManager = require("./cookieManager"); // 🔥 ADD THIS
...
class VideoDownloaderService {
  constructor() {
    ...
    this.initializeR2Client();
    this.startCleanupJob();
    this.logCookieStatus(); // 🔥 ADD THIS
  }

  // 🔥 NEW METHOD - Log cookie status on startup
  logCookieStatus() {
  ...
  // 🔥 NEW METHOD - Detect platform from URL
  ...
  // 🔥 SMART buildDownloadOptions - Cookies ONLY for YouTube
  buildDownloadOptions({ url, quality, format, audioOnly }) {
    const options = [];

    // 🔥 CRITICAL: Detect platform and add cookies ONLY for YouTube
    const platform = this.detectPlatformFromUrl(url);

    if (platform === "youtube") {
      const hasCookies = cookieManager.addCookieOptions(options);
      if (hasCookies) {
        console.log(`🍪 [YouTube] Using cookies for authentication`);
      } else {
        console.log(`⚠️  [YouTube] No cookies - bot detection may occur`);
      }
    } else {
      console.log(`🔓 [${platform}] Skipping YouTube cookies (not needed)`);
    }
  ...
  getUserFriendlyError(errorMessage) {
    const errorMap = {
      "sign in": "⚠️ YouTube bot detection. Please try again in 30 seconds.",
      bot: "⚠️ YouTube bot detection. Please try again in 30 seconds.",
      private: "This video is private and cannot be downloaded.",
      unavailable: "This video is no longer available.",
      removed: "This video has been removed.",
      "age-restricted":
  ...
  // 🔥 SMART getVideoMetadata - Cookies ONLY for YouTube
  async getVideoMetadata(url, platform) {
    try {
      const options = [
        "--dump-json",
      ...
      // 🔥 CRITICAL: Add cookies ONLY for YouTube
      const detectedPlatform = this.detectPlatformFromUrl(url);
      if (detectedPlatform === "youtube") {
        const hasCookies = cookieManager.addCookieOptions(options);
        if (hasCookies) {
          console.log(`🍪 [YouTube] Using cookies for metadata`);
        }
      }
  ...
  getServerStats() {
    const cookieStatus = cookieManager.getStatus(); // 🔥 ADD THIS
    return {
      activeDownloads: this.activeDownloads.size,
      maxConcurrent: this.maxConcurrentDownloads,
      r2Status: this.r2Working ? "operational" : "degraded",
