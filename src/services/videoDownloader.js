...
const platformDetector = require("./platformDetector");
const Download = require("../models/Download");
const cookieManager = require("./cookieManager"); // 🆕 ADDED
...
class VideoDownloaderService {
  ...
  async downloadVideo(options = {}) {
    ...
    try {
      ...
      // 🆕 ADDED: Normalize format based on platform
      ...
      console.log(
        `⚙️ Quality: ${quality}, Format: ${normalizedFormat}, Thumbnail: ${includeThumbnail ? "YES" : "NO"}`,
      );
  ...
  async performStreamingDownload(options) {
    ...
    // 🆕 ADDED: Get platform from detection
  ...
  // 🔥 UPDATED: Bulletproof download with automatic fallback
  async downloadAndUploadToR2(options) {
    const {
      url,
      quality,
    ...
    // ═══════════════════════════════════════════════════════════
    // YOUTUBE: Try multiple strategies with automatic fallback
    // ═══════════════════════════════════════════════════════════
    if (platform === "youtube") {
      // STRATEGY 1: With cookies + full format
      try {
        console.log(`🍪 [${downloadId}] Attempt 1: With cookies, full quality`);
        ...
        return await this.uploadToR2AndCleanup(
          tempFile,
          fileName,
          contentType,
          downloadId,
        ...
        console.log(
          `⚠️  [${downloadId}] Attempt 1 failed: ${error1.message.substring(0, 100)}`,
        );

        if (error1.message.includes("Requested format is not available")) {
          // STRATEGY 2: Without cookies + same format
          try {
            console.log(
              `🔄 [${downloadId}] Attempt 2: Without cookies, same quality`,
            );

            const ytDlpArgs2 = this.buildDownloadOptions({
              quality,
              format,
              audioOnly,
              platform,
            ...
            ytDlpArgs2.push("-o", tempFile);

            await this.ytDlp.execPromise([url, ...ytDlpArgs2]);

            console.log(`✅ [${downloadId}] Success without cookies!`);
            return await this.uploadToR2AndCleanup(
              tempFile,
              fileName,
              contentType,
            ...
            console.log(
              `⚠️  [${downloadId}] Attempt 2 failed: ${error2.message.substring(0, 100)}`,
            );

            // STRATEGY 3: Simple format (guaranteed to work)
            try {
              console.log(
                `🔄 [${downloadId}] Attempt 3: Simple format (best available)`,
              );
              ...
              await this.ytDlp.execPromise([url, ...ytDlpArgs3]);

              console.log(`✅ [${downloadId}] Success with simple format!`);
              return await this.uploadToR2AndCleanup(
                tempFile,
                fileName,
                contentType,
              ...
              console.error(`❌ [${downloadId}] All 3 attempts failed`);
              throw error3;
            }
          }
        } else {
    ...
    // ═══════════════════════════════════════════════════════════
    // OTHER PLATFORMS: Simple download (no cookies needed)
    // ═══════════════════════════════════════════════════════════
    else {
      const ytDlpArgs = this.buildDownloadOptions({
        quality,
  ...
  // 🆕 ADDED: Helper method for R2 upload and cleanup (DRY principle)
  async uploadToR2AndCleanup(
    tempFile,
    fileName,
    contentType,
    downloadId,
  ...
  // 🔥 UPDATED: Bulletproof local download with automatic fallback
  async performLocalDownload(options) {
    ...
    if (platform === "youtube") {
      try {
        console.log(`🍪 [${downloadId}] Attempt 1: With cookies`);
        ...
        console.log(
          `⚠️  [${downloadId}] Attempt 1 failed, trying without cookies...`,
        );

        if (error1.message.includes("Requested format is not available")) {
          try {
            console.log(`🔄 [${downloadId}] Attempt 2: Without cookies`);
            ...
            console.log(`🔄 [${downloadId}] Attempt 3: Simple format`);
    ...
    // ═══════════════════════════════════════════════════════════
    // OTHER PLATFORMS: Simple download
    // ═══════════════════════════════════════════════════════════
    else {
      console.log(`⬇️ [${downloadId}] Downloading to temp file...`);
  ...
  // 🔥 UPDATED: Smart download options with cookie control
  buildDownloadOptions({
    ...
    // ═══════════════════════════════════════════════════════════
      ...
      // Flexible format string with multiple fallbacks
      const formatString =
    ...
    // ═══════════════════════════════════════════════════════════
    // YOUTUBE-SPECIFIC (COOKIES + ANTI-BOT)
    // ═══════════════════════════════════════════════════════════
    if (platform === "youtube") {
      // Only add cookies if useCookies is true
      if (useCookies) {
  ...
  // 🔥 UPDATED: Metadata extraction with cookie support
  async getVideoMetadata(url, platform) {
    try {
      const options = [
        "--dump-json",
      ...
      // 🆕 ADDED: Cookies for YouTube metadata
      if (platform === "youtube") {
